subjects = [17]; % check 26
path = 'C:\Users\7piec\Box\00_BEAR_Lab\Projects\Eden Winslow\Ultrasound Features\Patch Data Saved\Stats by Patch';
newPatches = ["100", "225", "400", "900", "1225", "1600", "3600", "4900"]; %running for additional patches
nReps = 50;

for subj = subjects
    % check to see if path exists, error if not
    if ~exist(path, 'dir')
    ...
    for file = 1:length(files)
        ...
        % find files for the given subject number and load them one by one
        if contains(fileName, subjectTag) && any(contains(fileName, newPatches)) 
            ...
            %save(sprintf('C:\\Users\\Bear Lab\\Box\\00_BEAR_Lab\\Projects\\Eden Winslow\\Ultrasound Features\\Patch Data Saved\\Classification by Patch\\SHR%03d_%02dPatches_%02dBlockAvg_ForWrap.mat',numSub,numPatches, classify_data.avgBy),'classify_data');
            save(sprintf('C:\\Users\\7piec\\Box\\00_BEAR_Lab\\Projects\\Eden Winslow\\Ultrasound Features\\Patch Data Saved\\Classification by Patch\\SHR%03d_%02dPatches_%02dBlockAvg_ForWrap.mat',numSub,numPatches, classify_data.avgBy),'classify_data');
