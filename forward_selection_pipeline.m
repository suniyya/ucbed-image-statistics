...
save_pattern = 'C:\Users\7piec\Box\00_BEAR_Lab\Projects\Eden Winslow\Ultrasound Features\Patch Data Saved\Classification by Patch\SHR%03d_%02dPatches_%02dBlockAvg_ForWrap.mat';

patch_sizes = [100, 225, 400, 900, 1225, 1600, 3600, 4900];
patch_tags = string(patch_sizes) + "Patches";  % avoids matching 100 when filename has 1000Patches
...
for subject_id = subjects
    ...
    % Get all candidate files once, then filter by subject and patch tag
    for file_idx = 1:length(files)
        file_name = files(file_idx).name;

        % Keep only files for this subject and one of the target patch sizes
        ...
        result.imgLen = all_data.imgLen;  % length of crop
        result.centX = all_data.centX;    % center of crop (x, y)
        result.centY = all_data.centY;

        for arm_idx = 1:length(arm_names)
            arm = arm_names{arm_idx};

            all_x = all_data.(arm){1,1}.stats;  % [nTrials x numPatches x numStats]
            labels = categorical(all_data.(arm){1,1}.graspname);

            [~, num_patches, num_stats] = size(all_x);
            num_sub = all_data.(arm){1,1}.subj(1);

            % Run forward selection for this arm
            arm_result = run_forward_selection(all_x, labels, n_reps, num_folds, num_neighbors);
